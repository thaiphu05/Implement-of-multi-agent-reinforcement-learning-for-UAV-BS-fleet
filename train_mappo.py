...
def parse_args():
    ...
    parser.add_argument("--tranfer_train", type=str, default=False)
...
def transfer_train(file_pt, num_episodes):
    """Transfer learning from a saved .pt checkpoint for a given number of episodes."""
    cfg = parse_args()
    cfg.episodes = int(num_episodes)

    if not os.path.isfile(file_pt):
        raise FileNotFoundError(f"Checkpoint not found: {file_pt}")
    ...
    policy.load(file_pt, map_location=device, load_optimizers=False)
    print(f"Loaded transfer checkpoint from {file_pt}")
...
def main():
    cfg = parse_args()
    if cfg.tranfer_train:
        checkpoint_path = "mappo_checkpoint_final" + cfg.checkpoint_name + ".pt"
        if checkpoint_path is None:
            raise ValueError("Checkpoint path must be provided for transfer training")
        transfer_train(file_pt=checkpoint_path, num_episodes=cfg.episodes)
    else:
        train()
